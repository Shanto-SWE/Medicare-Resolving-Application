...
//    Doctors
    ...
    //    email check
        $emailquery="select * from doctors where email='$email'";
        $query=$this->conn->query($emailquery);
        $eamilcount=mysqli_num_rows($query);

        //  username check
        $usernamequery="select * from doctors where username='$username'";
        $userNamequery=$this->conn->query($usernamequery);
        $usernamecount=mysqli_num_rows($userNamequery);

        // user id check
        $useridquery="select * from doctors where user_id='$userid'";
        $userIdquery=$this->conn->query($useridquery);
        $useridcount=mysqli_num_rows($userIdquery);
           ...
           if($eamilcount>0){
            ...
            return $message;
               ...
               if($usernamecount>0){
                $message = '<span style="color:red">User Name Already Exist.</span>';
                return $message;
               }else{
                if($useridcount>0){
                    $message = '<span style="color:red">User Id Already Exist.</span>';
                    return $message;
                }else{
...
public function pharmacistAuthData()
{
...
//    email check
    $emailquery="select * from pharmacist where email='$email'";
    $query=$this->conn->query($emailquery);
    $eamilcount=mysqli_num_rows($query);

    //  username check
    $usernamequery="select * from pharmacist where user_name='$username'";
    $userNamequery=$this->conn->query($usernamequery);
    $usernamecount=mysqli_num_rows($userNamequery);

    // user id check
    $useridquery="select * from pharmacist where user_id='$userid'";
    $userIdquery=$this->conn->query($useridquery);
    $useridcount=mysqli_num_rows($userIdquery);
        ...
        return $message;
           ...
           if($usernamecount>0){
            ...
            if($useridcount>0){
                ...
                $sql = "INSERT INTO `pharmacist`(`first_name`,`last_name`, `user_name`, `user_id`, `password`, `email`, `phone`,`role`,`status`) VALUES ('$fname','$lname','$username','$userid','$password','$email','$phone','$role',1)";
            $insertquery=$this->conn->query($sql);
            $message = '<span style="color:green">Pharmacist Create successfully.</span>';
            return $message;
            }
           }
...
public function viewPharmacistlist()
{
...
public function pharmacistChangeStatus($data)
    {
        $sql = "UPDATE `pharmacist` SET `status`='$data[status]' WHERE user_id = '$data[pharmacist_id]' ";
        if ($this->conn->query($sql) === TRUE) {
            echo "<script type='text/javascript'>alert('UPDATED!');document.location='pharmacist.php';</script>";
        } else {
            return $message = 'ERROR:' . $this->conn->error;
        }
    }

    public function viewPharmacistData($data)
    {

        $sql = "SELECT *FROM `pharmacist` WHERE user_id = '$data[pharmacist_id]'";
        $result = $this->conn->query($sql);

        return $result;
    }
    // update pharmacist data
    public function updatePharamacist($data)
    {
     


        $directory = '../gallery/propic/pharamacist/';
        ...
        if ($this->conn->query($sql) === TRUE) {
            echo "<script type='text/javascript'>alert('DATA UPDATED SUCCESSFULLY');document.location='pharmacist.php';</script>";
        } else {
            return $message = 'ERROR:' . $this->conn->error;
        }
    }
    public function deletePharmacist($data)
    {

        $sql = "DELETE FROM `pharmacist` WHERE id = '$data[id]'";
    ...
    public function UpdateClientData($data)
    {
        ...
        if ($this->conn->query($sql) === TRUE) {
            echo "<script type='text/javascript'>alert('DATA UPDATED SUCCESSFULLY');document.location='clients.php';</script>";
        } else {
            return $message = 'ERROR:' . $this->conn->error;
        }
    }
